...
namespace xayah::operators::vorticity::cuda {
    __global__ void compute_vorticity_kernel(float* omega_x, float* omega_y, float* omega_z, float* omega_magnitude, const float* velocity_x, const float* velocity_y, const float* velocity_z, const std::uint32_t* cell_indices, const int nx, const int ny, const int nz, const float h, const core::boundary::cuda::VectorBoundary3D boundary_config) {
        ...
        if (core::boundary::cuda::cell_is_marked(cell_indices, x, y, z, nx, ny, nz, boundary_config)) {
            omega_x[index]         = 0.0f;
            omega_y[index]         = 0.0f;
            omega_z[index]         = 0.0f;
        ...
        const float dvz_dy = 0.5f * (core::boundary::cuda::load_centered_component(velocity_z, 2u, x, y + 1, z, nx, ny, nz, boundary_config) - core::boundary::cuda::load_centered_component(velocity_z, 2u, x, y - 1, z, nx, ny, nz, boundary_config)) / h;
        const float dvy_dz = 0.5f * (core::boundary::cuda::load_centered_component(velocity_y, 1u, x, y, z + 1, nx, ny, nz, boundary_config) - core::boundary::cuda::load_centered_component(velocity_y, 1u, x, y, z - 1, nx, ny, nz, boundary_config)) / h;
        const float dvx_dz = 0.5f * (core::boundary::cuda::load_centered_component(velocity_x, 0u, x, y, z + 1, nx, ny, nz, boundary_config) - core::boundary::cuda::load_centered_component(velocity_x, 0u, x, y, z - 1, nx, ny, nz, boundary_config)) / h;
        const float dvz_dx = 0.5f * (core::boundary::cuda::load_centered_component(velocity_z, 2u, x + 1, y, z, nx, ny, nz, boundary_config) - core::boundary::cuda::load_centered_component(velocity_z, 2u, x - 1, y, z, nx, ny, nz, boundary_config)) / h;
        const float dvy_dx = 0.5f * (core::boundary::cuda::load_centered_component(velocity_y, 1u, x + 1, y, z, nx, ny, nz, boundary_config) - core::boundary::cuda::load_centered_component(velocity_y, 1u, x - 1, y, z, nx, ny, nz, boundary_config)) / h;
        const float dvx_dy = 0.5f * (core::boundary::cuda::load_centered_component(velocity_x, 0u, x, y + 1, z, nx, ny, nz, boundary_config) - core::boundary::cuda::load_centered_component(velocity_x, 0u, x, y - 1, z, nx, ny, nz, boundary_config)) / h;
    ...
    __global__ void add_confinement_kernel(float* destination_x, float* destination_y, float* destination_z, const float* omega_x, const float* omega_y, const float* omega_z, const float* omega_magnitude, const std::uint32_t* cell_indices, const int nx, const int ny, const int nz, const float h, const float epsilon, const core::boundary::cuda::VectorBoundary3D boundary_config) {
        const int x = static_cast<int>(blockIdx.x * blockDim.x + threadIdx.x);
        const int y = static_cast<int>(blockIdx.y * blockDim.y + threadIdx.y);
        const int z = static_cast<int>(blockIdx.z * blockDim.z + threadIdx.z);
        if (x >= nx || y >= ny || z >= nz) return;
        if (core::boundary::cuda::cell_is_marked(cell_indices, x, y, z, nx, ny, nz, boundary_config)) return;

        const float grad_x   = 0.5f * (core::boundary::cuda::load_centered_scalar(omega_magnitude, x + 1, y, z, nx, ny, nz, boundary_config) - core::boundary::cuda::load_centered_scalar(omega_magnitude, x - 1, y, z, nx, ny, nz, boundary_config)) / h;
        const float grad_y   = 0.5f * (core::boundary::cuda::load_centered_scalar(omega_magnitude, x, y + 1, z, nx, ny, nz, boundary_config) - core::boundary::cuda::load_centered_scalar(omega_magnitude, x, y - 1, z, nx, ny, nz, boundary_config)) / h;
        const float grad_z   = 0.5f * (core::boundary::cuda::load_centered_scalar(omega_magnitude, x, y, z + 1, nx, ny, nz, boundary_config) - core::boundary::cuda::load_centered_scalar(omega_magnitude, x, y, z - 1, nx, ny, nz, boundary_config)) / h;
