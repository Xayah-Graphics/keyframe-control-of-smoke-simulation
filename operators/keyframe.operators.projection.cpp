...
namespace kfs::operators {
    ...
    void Projection::operator()(field::StaggeredVectorField3D& destination, field::StaggeredVectorField3D& working, const field::CenteredVectorField3D& solid_velocity, const std::uint8_t* occupancy, const float delta_seconds) {
        ...
        cuda::operators::projection::build_pressure_matrix(this->stream, this->pressure_values, this->pressure_row_offsets, this->pressure_column_indices, occupancy, this->pressure_anchor, nx, ny, nz, flow_types);
        if (const cudaError_t status = cudaMemsetAsync(this->pressure, 0, bytes, this->stream); status != cudaSuccess) throw std::runtime_error{std::string{"cudaMemsetAsync projection pressure: "} + cudaGetErrorString(status)};
