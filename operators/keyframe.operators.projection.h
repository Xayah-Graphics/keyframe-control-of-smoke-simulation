...
namespace kfs::cuda::operators::projection {
    ...
    void compute_pressure_rhs(cudaStream_t stream, float* rhs, const float* velocity_x, const float* velocity_y, const float* velocity_z, const std::uint8_t* occupancy, const int* pressure_anchor, int nx, int ny, int nz, float h, float dt, const std::uint32_t* flow_boundary_types, const float* flow_boundary_pressure);
    void build_pressure_matrix(cudaStream_t stream, float* values, const int* row_offsets, const int* column_indices, const std::uint8_t* occupancy, const int* pressure_anchor, int nx, int ny, int nz, const std::uint32_t* flow_boundary_types);
